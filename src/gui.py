...
def h_layers_info():
    messagebox.showinfo(
        "Hidden layers",
        "Hidden layers are written as integers separated by spaces. For example, the input '2 8 16' " +
...
def gui(runner):
    ...
    tk.Checkbutton(root, text=" Use Bias", onvalue=1, offvalue=0, variable=bias_var).grid(column=0, row=6)

    logging_widget = LoggingWidget(root, 3, 7, "logs/run.log")
